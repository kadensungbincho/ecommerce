...
class OrderManagerQuerySet(models.query.QuerySet):
    ...
    def by_weeks_range(self, weeks_ago=1, number_of_weeks=1):
        ...
        day_ago_end = (weeks_ago - number_of_weeks) * 7
        start_date = timezone.now() - datetime.timedelta(days=days_ago_start)
        end_date = timezone.now() - datetime.timedelta(days=day_ago_end)
...
class Order(models.Model):
    ...
    def update_purchases(self):
        for p in self.cart.products.all():
            obj, creatd = ProductPurchase.objects.get_or_create(
...
class ProductPurchageManager(models.Manager):
    def get_queryset(self):
...
class ProductPurchase(models.Model):
    ...
    billing_profile = models.ForeignKey(BillingProfile, null=True, blank=True, on_delete=models.CASCADE) # billingprofile.productpurchase_set.all()
    product         = models.ForeignKey(Product, on_delete=models.CASCADE) # product.productpurchase_set.count()
    refunded        = models.BooleanField(default=False)
    updated         = models.DateTimeField(auto_now=True)
    timestamp       = models.DateTimeField(auto_now_add=True)

    objects = ProductPurchageManager()
